<template>
      <div class="item-box" style="float:left;">
        <img :alt="item.name" :src=" item.sku_info[itemIndex].ali_image+'?x-oss-process=image/resize,w_206/quality,Q_80/format,webp' " style="opacity: 1;"
        >
        <h6>{{item.name}}</h6>
        ...
        <div class="params-colors">
          <ul class="colors-list">
            <li key="index" v-for="sku,index in item.sku_info">
              <a href="javascript:;" @click="tableData(index)" :data-index="index" :title="sku.spec_json.show_name" :class="{'active':index==itemIndex}"><img :src="'http://img01.smartisanos.cn/'+ sku.spec_json.image +'/20X20.jpg'" style="float:left"></a>
            </li>
          </ul>
        </div>
