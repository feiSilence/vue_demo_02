...
<div id="main">
    <div>这个是item详情页</div>
    <div>{{itemsInfo.title}}</div>
    <div>￥{{itemsInfo.price}}</div>

    <div class="gallery-wrapper">
        <div class="gallery">
            <div class="thumbnail">
                <ul>
                    <li :class="{'on':index==imgIndex}" @click="tableImg(index)" key="index" v-for="img,index in itemsInfo.ali_images"><img :src="img+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
                </ul>
            </div>
            <div class="thumb">
                <ul>
                    <li :class="{'on':index==imgIndex}" key="index" v-for="img,index in itemsInfo.ali_images"><img :src="img+'?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'"></li>
                </ul>
            </div>
        </div>
    </div>
...
export default {
  ...
  //过滤详情页数据itemsList，数组数据，在页面中渲染对应id的item数据{{itemsInfo.title}}
  computed: {
    itemsInfo () {
